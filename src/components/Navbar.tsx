            ...
            <div className="flex items-center gap-2">
              {user && (
                <div className="relative">
                  <button onClick={() => { setShowNotif(!showNotif); setShowProfile(false); }}
                    className={`w-9 h-9 rounded-full flex items-center justify-center transition-all duration-300 ${showNotif ? 'bg-orange-100/60 text-orange-600 scale-110' : 'text-stone-400 hover:bg-white/40 hover:text-stone-600 hover:scale-105'}`}>
                    ...
                    <div className="absolute top-1 right-1 w-2.5 h-2.5 rounded-full bg-orange-500 border-2 border-white/60 animate-pulse" />
                  </button>
              ...
              {user ? (
                    ...
                    <div className="absolute top-12 right-0 w-56 rounded-2xl shadow-xl overflow-hidden animate-fade-scale z-50" style={{
                      ...
                      <div className="py-1.5">
                        <Link href="/cabinet" onClick={() => setShowProfile(false)} className="flex items-center gap-3 px-4 py-2.5 text-xs font-semibold text-stone-700 hover:bg-orange-50/60 hover:text-orange-600 transition-all">
                          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>Kabinet
                        </Link>
                        {(user.role === 'TEACHER' || user.role === 'ADMIN') && (
                          <Link href="/teacher" onClick={() => setShowProfile(false)} className="flex items-center gap-3 px-4 py-2.5 text-xs font-semibold text-stone-700 hover:bg-purple-50/60 hover:text-purple-600 transition-all">
                            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg>O&apos;qituvchi paneli
                          </Link>
                        )}
                        {user.role === 'ADMIN' && (
                          <Link href="/admin" onClick={() => setShowProfile(false)} className="flex items-center gap-3 px-4 py-2.5 text-xs font-semibold text-stone-700 hover:bg-orange-50/60 hover:text-orange-600 transition-all">
                            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="3"/><path d="M12.22 2h-.44a2 2 0 00-2 2v.18a2 2 0 01-1 1.73l-.43.25a2 2 0 01-2 0l-.15-.08a2 2 0 00-2.73.73l-.22.38a2 2 0 00.73 2.73l.15.1a2 2 0 011 1.72v.51a2 2 0 01-1 1.74l-.15.09a2 2 0 00-.73 2.73l.22.38a2 2 0 002.73.73l.15-.08a2 2 0 012 0l.43.25a2 2 0 011 1.73V20a2 2 0 002 2h.44a2 2 0 002-2v-.18a2 2 0 011-1.73l.43-.25a2 2 0 012 0l.15.08a2 2 0 002.73-.73l.22-.39a2 2 0 00-.73-2.73l-.15-.08a2 2 0 01-1-1.74v-.5a2 2 0 011-1.74l.15-.09a2 2 0 00.73-2.73l-.22-.38a2 2 0 00-2.73-.73l-.15.08a2 2 0 01-2 0l-.43-.25a2 2 0 01-1-1.73V4a2 2 0 00-2-2z"/></svg>Admin panel
                          </Link>
                        )}
                        ...
                        <button onClick={() => { logout(); setShowProfile(false); }} className="flex items-center gap-3 px-4 py-2.5 text-xs font-semibold text-red-500 hover:bg-red-50/60 transition-all w-full text-left">
                          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>Chiqish
                        </button>
                      </div>
                    </div>
                  )}
                ...
                <div className="flex gap-2 ml-1">
                  <button onClick={() => setAuthMode('login')} className="btn-glass text-xs font-bold text-stone-700 px-4 py-2 rounded-full">Kirish</button>
                  <button onClick={() => setAuthMode('register')} className="btn-primary text-xs font-bold text-white px-4 py-2 rounded-full">Ro&apos;yxatdan o&apos;tish</button>
                </div>
              )}
            </div>
          </div>
