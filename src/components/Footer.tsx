...
export default function Footer() {
  const [socials, setSocials] = useState<any>({ telegram: '#', instagram: '#', youtube: '#' });
  const [location, setLocation] = useState<any>({ address: 'Toshkent, O\'zbekiston', mapUrl: '' });

  useEffect(() => {
    api.getSiteContent().then((d: any) => {
      if (d.footer_socials) setSocials(d.footer_socials);
      if (d.footer_location) setLocation(d.footer_location);
    }).catch(() => {});
  }, []);
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10 mb-10">
          ...
          <div>
            ...
            <div className="flex gap-3">
              <a href={socials.telegram} target="_blank" rel="noopener" className="w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-300 hover:scale-110 hover:shadow-lg" style={{ background: 'rgba(0,136,204,0.1)' }}>
              ...
              <a href={socials.instagram} target="_blank" rel="noopener" className="w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-300 hover:scale-110 hover:shadow-lg" style={{ background: 'rgba(225,48,108,0.1)' }}>
              ...
              <a href={socials.youtube} target="_blank" rel="noopener" className="w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-300 hover:scale-110 hover:shadow-lg" style={{ background: 'rgba(255,0,0,0.1)' }}>
          ...
          <div>
            ...
            {location.mapUrl ? (
              <div className="rounded-xl overflow-hidden border border-white/40" style={{ boxShadow: '0 4px 15px rgba(0,0,0,0.06)' }}>
                <iframe src={location.mapUrl} width="100%" height="140" style={{ border: 0 }} allowFullScreen loading="lazy" referrerPolicy="no-referrer-when-downgrade" />
              </div>
            ) : (
              <div className="h-[140px] rounded-xl flex items-center justify-center text-xs text-stone-400" style={{ background: 'rgba(255,255,255,0.4)' }}>
                Xarita yuklanmadi
              </div>
            )}
          </div>
