...
export default function UsefulPage() {
  ...
  useEffect(() => {
    api.getSiteContent().then((d: any) => {
      if (d.useful_videos) setVideos(d.useful_videos);
    }).catch(() => {});
  }, []);
    ...
    <div className="px-4 py-16 max-w-4xl mx-auto">
      <div className="text-center mb-12">
        ...
        <h1 className="text-3xl md:text-4xl font-black tracking-tight">
          Foydali <span className="bg-gradient-to-r from-orange-500 to-amber-500 bg-clip-text text-transparent">videolar</span>
        </h1>
        <p className="text-sm text-stone-500 mt-3">O&apos;quv markazimiz haqida batafsil videolar</p>
      </div>

      {videos.length === 0 ? (
        <div className="text-center py-16 text-stone-400 text-sm">Videolar hali qo&apos;shilmagan</div>
      ) : (
          ...
          {videos.map((v, i) => (
            <div key={i} className="rounded-2xl overflow-hidden" style={{
              ...
              <div className="aspect-video">
                <iframe src={v.url} width="100%" height="100%" style={{ border: 0 }}
